...
function DirectRouteCard({ route }) {
  ...
  const firstStop =
    routeStops.length > 0
      ? routeStops[0]?.stop || routeStops[0]
      : null;

  const lastStop =
    routeStops.length > 0
      ? routeStops[routeStops.length - 1]?.stop ||
...
function TransferRouteCard({ route }) {
  ...
  const firstLegStops = firstLeg?.stops || [];
  const lastLegStops = lastLeg?.stops || [];

  const firstStop =
    firstLegStops.length > 0
      ? firstLegStops[0]?.stop || firstLegStops[0]
      : null;

  const lastStop =
    lastLegStops.length > 0
      ? lastLegStops[lastLegStops.length - 1]?.stop ||
      lastLegStops[lastLegStops.length - 1]
      : null;
